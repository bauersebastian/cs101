...
def date_converter(dict, string):
    month_pos = string.find('/')
    month_endpos = string.find('/',month_pos+1)
    month = int(string[:month_pos])
    month_letter = dict[month]
    result = string[month_pos+1:month_endpos] + " " + month_letter + " " + string[month_endpos+1:]
    return result

def date_converter_short(dict,string):
    month, day, year = string.split('/')
    return day + " " + dict[int(month)] + " " + year


print date_converter_short(english, '5/11/2012')
#>>> 11 May 2012
